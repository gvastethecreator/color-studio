...
export function Slider({
  ...
  max = 100,
  ...props
}: SliderPrimitive.Root.Props & { trackClassName?: string }): React.ReactElement {
  ...
  if (thumbIdsRef.current.length !== _values.length) {
    thumbIdsRef.current = _values.map(() => crypto.randomUUID().slice(0, 8));
  }
        ...
        >
          ...
          {_values.map((val, position) => (
            <SliderPrimitive.Thumb
              ...
              index={position}
              key={thumbIdsRef.current[position]}
            />
          ))}
        </SliderPrimitive.Track>
      </SliderPrimitive.Control>
